    ...
    for km in range(int(distance_km)):
        ...
        if len(indices) > 0:
            ...
            for i in range(len(indices)-1):
                segment_distance = (distances_km[indices[i+1]] - distances_km[indices[i]]) * 1000
            ...
            km_pace_adjusted = km_pace * fatigue_factor(km+1, total_distance) * strategy_factor(km+1, total_distance, slopes, distances_km)
            
            km_time = km_pace_adjusted
            km_times.append(km_time)
            km_numbers.append(km + 1)
